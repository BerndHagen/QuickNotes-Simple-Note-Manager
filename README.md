...
- **Document Objects:** Open one compact ribbon control for a dense Word-style gallery of 46 categorized SVG shapes, then move, resize from eight handles, rotate, flip, wrap, recolor, and size them precisely
...
- **`ShapeExtension.js`** — Adds 46 editable SVG objects through one compact categorized gallery with recently used shapes, drag creation, eight-handle resize, exact geometry, rotation, flips, wrapping, keyboard movement, and color presets.
